...
//		let FechaDesde =document.getElementById("txtfechadesde").value
	...
	debugger
	$('#tbListadoUsuarios').DataTable().destroy();
	$("#tbListadoUsuarios").DataTable({
		"language": {
		...
		"ajax": {
			type: 'POST',
			url: "../UsuarioValidacion?accion=ListarUsuarios",
			data:
				{
					json: JSON.stringify(json)
		...
		"columnDefs": [
			...
			{targets: 2, orderable: false, width: "5%", className: "text-center"},
...
//   loader.terminarLoader()
	...
	$('#tbListadoUsuarios').on('draw.dt', function () {
		$('[data-toggle="tooltip"]').tooltip();
	});
}
...
$('#tbListadoUsuarios tbody').on('click', '.editarTrabajador', function () {
	var datos = $('#tbListadoUsuarios').DataTable().row($(this).parents('tr')).data();
	codigo = datos[2];
